...
import themeQoraiDefault from '../../src/theme/qorai-default'
...
const allQoraiDefaultItems: any = []
for (const name in themeQoraiDefault.color) {
  // @ts-ignore
  allQoraiDefaultItems.push({ name, color: themeQoraiDefault.color[name] })
}
  ...
  .add('Theme Variables', () => {
    return (
      <Palette individual={true}>
        {allQoraiDefaultItems.map(({ color, name }: any) =>
